...
#   -v   Verbose    — print each rename/merge on its own line (disables in-place counter)
...
progress() {
    printf "\rProgress: Checked %-8s folders, renamed %-8s folders" "$1" "$2"
}

# Escape glob metacharacters so find -name treats the string as a literal.
# Affects: [ ] * ?
escape_glob() {
    local s="$1"
    s="${s//\\/\\\\}"   # backslash first to avoid double-escaping
    s="${s//\[/\\[}"
    s="${s//\]/\\]}"
    s="${s//\*/\\*}"
    s="${s//\?/\\?}"
...
main() {
    ...
    # Escape metacharacters so find -name matches literally
    local escaped_src
    escaped_src="$(escape_glob "$SRC")"

    local find_args=( find "$SEARCH_ROOT" -depth -type d -name "$escaped_src" )
    ...
    while IFS= read -r -d '' found_path; do
        ...
        $VERBOSE || progress "$checked" "$renamed"

    done < <("${find_args[@]}" -print0 2>/dev/null)

    $VERBOSE || echo
    info "Done: checked ${checked} folders, renamed ${renamed}."
}
