...
# In-place progress line — overwrites itself using carriage return
progress() {
...
main() {
    ...
    while IFS= read -r -d '' found_path; do
        ...
        if [[ -e "$dest" ]]; then
            ...
            if ! conflict="$(find_conflict "$found_path" "$dest")"; then
                if $OVERWRITE; then
                    ...
                    # Print a newline so the error appears below the progress line
                    $VERBOSE || echo
        ...
        # Update in-place counter unless -v is on (verbose scrolls, counter would thrash)
        $VERBOSE || progress "$checked" "$renamed"

    done < <("${find_args[@]}" -print0 2>/dev/null)

    # Final newline to step off the progress line, then summary
    $VERBOSE || echo
